...
# --- Dropdown output selection ---
    ...
    if filtered_chars:
        ...
        if (
            ...
            and st.session_state.previous_selected_comp != st.session_state.selected_comp
            and st.session_state.previous_selected_comp not in filtered_chars
            and st.session_state.previous_selected_comp in component_map
        ):
        ...
        st.selectbox(
            ...
            if c == "Select a character..."
            else f"{c} ({clean_field(component_map[c]['meta'].get('pinyin'))}, {get_stroke_count(c)} strokes, {clean_field(component_map[c]['meta'].get('definition'))})"
        )
