...
def init_session():
    if "selected_comp" not in st.session_state:
        default = random.choice(list(component_map.keys()))
        st.session_state.selected_comp = default
        st.session_state.previous_selected_comp = default
        ...
        st.session_state.text_input_comp = default
...
def render_controls():
    ...
    with col1:
        selected = st.selectbox("Select a component:", all_components,
    ...
    with col2:
        st.text_input("Or type a component:", st.session_state.text_input_comp,
                      key="text_input_comp", on_change=lambda: update_selected_comp(st.session_state.text_input_comp))
    with col3:
        idcs = {"No Filter"} | {component_map[c]["meta"].get("IDC", "—") for c in component_map.get(st.session_state.selected_comp, {}).get("related_characters", [])}
        st.session_state.selected_idc = st.selectbox("IDC Filter", sorted(idcs), index=0)
...
summary = " ".join(f"<strong>{k}:</strong> {v}  " for k, v in fields.items())
st.markdown(f"<div class='selected-card'><h2 class='selected-char'>{st.session_state.selected_comp}</h2><p class='details'>{summary}</p></div>", unsafe_allow_html=True)
...
for c in filtered_chars:
    ...
    if st.session_state.display_mode == "Single Character":
        ...
        char_compounds[c] = [p for p in compounds if len(p) == length]

filtered_chars = [c for c in filtered_chars if char_compounds[c] or st.session_state.display_mode == "Single Character"]

# --- Output Dropdown ---
if filtered_chars:
    dropdown_options = ["Select a character..."] + sorted(filtered_chars, key=get_stroke_count)
    st.selectbox("Choose a character from output:", dropdown_options, key="output_char_select")
    if st.session_state.output_char_select != "Select a character...":
        update_selected_comp(st.session_state.output_char_select)
